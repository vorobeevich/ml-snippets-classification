...
class code4ml(BaseDataset):
    ...
    def split_data(self) -> None:
        snippets_train, snippets_test, labels_train, labels_test = train_test_split(
            self.snippets, self.labels, test_size=0.4, random_state=self.seed)
        snippets_train, snippets_val, labels_train, labels_val = train_test_split(
            snippets_train, labels_train, test_size=0.33, random_state=self.seed)
